...
local FastCast = {}
local Signal = require(script:WaitForChild("Signal")) --Unsure about this module? https://roblox.com/library/2530470096/rbxscriptsignal -- It's open source! Feel free to read it yourself.
...
function FastCast.new()
	...
	--Fire with physics.
	local function MainCastFire(Origin, Direction, Velocity, Function, CosmeticBulletObject, WhizTable, List)
...
--			local Change = (LastToCurrent_Distance - EndToCurrent_Distance)
			...
			LengthChanged:Fire(Origin, LastPoint, RayDir.Unit, LastToCurrent_Distance, CosmeticBulletObject, WhizTable)
			LastPoint = At
			if Hit and Hit.Transparency < 0.9 then
				--Make sure the part isn't a accessory, and ignore it if it is (added because accessories were OP armor)
			    if (Hit.Parent and not Hit.Parent:IsA("Accoutrement")) then
	...
	local function MainCastFireNoPhys(Origin, Direction, Velocity, Function, CosmeticBulletObject, WhizTable, List)
		if type(Velocity) == "number" then
			Velocity = Direction.Unit * Velocity
		end
		...
		while DistanceTravelled <= Distance do
			...
			LengthChanged:Fire(Origin, LastPoint, RayDir.Unit, ExtraDistance, CosmeticBulletObject, WhizTable)
	...
	function Caster:Fire(Origin, Direction, Velocity, CosmeticBulletObject, WhizTable, NoThread)
		...
		if NoThread then
			warn("NoThread=true is deprecated an will be removed in the future. Use Caster.RayHit:Wait() to yield instead.")
			if UsingPhysics() then
				MainCastFire(Origin, Direction, Velocity, Cast, CosmeticBulletObject, WhizTable)
			else
				MainCastFireNoPhys(Origin, Direction, Velocity, Cast, CosmeticBulletObject, WhizTable)
			end
		else
			spawn(function ()
				if UsingPhysics() then
	...
	function Caster:FireWithWhitelist(Origin, Direction, Velocity, Whitelist, CosmeticBulletObject, WhizTable, NoThread)
		--Note to scripters: 'self' is a variable lua creates when a method like ^ is run. It's an alias to the table that the function is part of (in this case, Caster)
		assert(Caster == self, "Expected ':' not '.' calling member function FireWithWhitelist")
		if NoThread then
			warn("NoThread=true is deprecated an will be removed in the future. Use Caster.RayHit:Wait() to yield instead.")
			if UsingPhysics() then
				MainCastFire(Origin, Direction, Velocity, CastWhitelist, CosmeticBulletObject, WhizTable, Whitelist)
			else
				MainCastFireNoPhys(Origin, Direction, Velocity, CastWhitelist, CosmeticBulletObject, WhizTable, Whitelist)
			end
		else
			spawn(function ()
				if UsingPhysics() then
	...
	function Caster:FireWithBlacklist(Origin, Direction, Velocity, Blacklist, CosmeticBulletObject, WhizTable, NoThread)
		...
		if NoThread then
			warn("NoThread=true is deprecated an will be removed in the future. Use Caster.RayHit:Wait() to yield instead.")
			if UsingPhysics() then
				MainCastFire(Origin, Direction, Velocity, CastBlacklist, CosmeticBulletObject, WhizTable, Blacklist)
			else
				MainCastFireNoPhys(Origin, Direction, Velocity, CastBlacklist, CosmeticBulletObject, WhizTable, Blacklist)
			end
		else
			spawn(function ()
				if UsingPhysics() then
